...
.Parameter AZP
    Running this script as a part of Azure Pipelines.
...
# Asynchronously starts a single test case running.
...
# Asynchronously start all the test cases running.
...
function PrintLldbCoreCallStack($CoreFile) {
    try {
        ...
        for ($i = 0; $i -lt $Output.Length; $i++) {
            ...
            if ($Output[$i] -like "*cxplat_bugcheck*") {
                $Found = $true
...
function Wait-TestCase($TestCase) {
    ...
    try {
        ...
        if ($KeepOutputOnSuccess -or $ProcessCrashed -or $AnyTestFailed) {
            ...
            if ($LogProfile -ne "None") {
                & $LogScript -Stop -OutputPath (Join-Path $TestCase.LogDir "cxplat")
            }
...
if ($Kernel -ne "") {
    if ($null -ne (Get-Service -Name "cxplattest" -ErrorAction Ignore)) {
        try { net.exe stop cxplattest /y | Out-Null } catch {}
        sc.exe delete cxplattest /y | Out-Null
    }
    Copy-Item (Join-Path $Kernel "cxplattest.sys") $DriverPath -Force

    if ($EnableSystemVerifier) {
        verifier.exe /volatile /adddriver cxplattest.sys /flags 0x9BB
        if ($LastExitCode) {
...
try {
    ...
    # Uninstall the kernel mode test driver.
    if ($Kernel -ne "") {
        try { net.exe stop cxplattest /y | Out-Null } catch {}
        sc.exe delete cxplattest | Out-Null
        Remove-Item (Join-Path $DriverPath cxplattest.sys) -Force
    }

    if ($IsWindows -and $EnableSystemVerifier) {
        if ($Kernel -ne "") {
            verifier.exe /volatile /removedriver cxplattest.sys
